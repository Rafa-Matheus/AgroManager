...
namespace EmbeddedExcel
{
    public partial class ExcelWrapper : UserControl
    {
        ...
        public void OpenFile(string filename)
        {
            ...
            //WebBrowserExcel.Navigating += (o, args) =>
            //{

            //};


            WebBrowserExcel.Url = new Uri(string.Format("file:///{0}", filename));
            //WebBrowserExcel.Document.ExecCommand("Refresh", false, "");
        ...
        private void AttachApplication()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(m_ExcelFileName)) return;
        ...
        public Worksheet FindExcelWorksheet(string sheetname)
        {
            ...
            {
                sheet = (_Worksheet)m_Workbook.Sheets.get_Item((object)isheet);

                sheet = (_Worksheet)m_Workbook.Sheets.get_Item(0);
        ...
        public void Print()
        {
            if (WebBrowserExcel.Url != null)
            {
                WebBrowserExcel.Focus();
                SendKeys.SendWait("^(p)");
            }
        }
        ...
        public void Replace(string match, string value)
        {
            if (WebBrowserExcel.Url != null)
            {
                for (int k = 0; k < m_Workbook.Sheets.Count; k++)
                {
                    Worksheet sheet = m_Workbook.Sheets[k + 1] as Worksheet;
